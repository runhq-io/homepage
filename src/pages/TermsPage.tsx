...
const TERMS_T = {
  ...
  ko: {
    ...
    li2_2Label: '고객 콘텐츠',
    li2_2Body: ' — 귀하가 서비스에 제출하는 할 일 항목, 코드, 댓글, 첨부 파일, 프롬프트, agent 입력 및 출력, 기타 모든 데이터를 의미합니다.',
    li2_3Label: '승인된 사용자',
    ...
    li6_2Label: 'agent 크레딧.',
    li6_2Body: ' agent 실행은 당사가 공시하는 요율로 크레딧을 소모합니다. 크레딧은 각 청구 주기 시작 시 갱신되며, 귀하의 요금제에 명시된 경우를 제외하고는 이월되지 않습니다.',
    li6_3Label: '초과 사용.',
    ...
    // 11. Coding agents and generated output
    s11H: '11. 코딩 agent 및 생성된 출력',
    s11p1Pre: '본 서비스는 귀하를 대신하여 AI 코딩 agent(예: Anthropic Claude 또는 OpenAI Codex)를 조율합니다. AI 출력은 확률적입니다: 부정확하거나 안전하지 않거나 귀하의 목적에 부적합할 수 있습니다. ',
    s11p1Strong: '귀하는 agent가 생성한 코드 또는 기타 산출물을 병합, 배포하거나 신뢰하기 전에 이를 검토, 테스트 및 검증할 단독 책임이 있습니다.',
    s11p2: '당사는 agent 출력이 독창적이거나 비침해적이거나 특정 목적에 적합함을 보증하지 않습니다. 기반 AI 제공업체의 약관이 특정 약속을 전달하거나 특정 권리를 제한하도록 요구하는 경우, 해당 약관은 참조로 통합되며 요청 시 제공됩니다.',
    ...
    // 18. Indemnification
    s18H: '18. 면책 및 보상',
    s18p1: '귀하는 (a) 고객 콘텐츠, (b) 귀하 또는 귀하의 승인된 사용자의 본 약관 또는 관련 법률을 위반한 서비스 이용, 또는 (c) 귀하의 agent 생성 출력 이용에서 발생하거나 이와 관련된 제3자 클레임으로부터 RunHQ와 그 임원, 이사, 직원 및 대리인을 방어, 면책 및 보호합니다. 당사는 그러한 클레임을 즉시 귀하에게 통지하고, 방어 및 합의에 대한 단독 통제권을 귀하에게 부여하며(당사의 인정 또는 지불을 요구하지 않는 합의에 한함), 합리적인 협조를 제공합니다.',
