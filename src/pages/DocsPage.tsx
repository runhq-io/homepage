...
const DOCS_T = {
  ...
  ko: {
    ...
    welcomeHowAgents: 'agent가 작동하는 방식',
...
const DOC_CARDS_KO: DocCard[] = [
  ...
  { t: 'agent 실행',              d: '할 일에서 Run을 누르세요. Claude나 Codex가 즉시 이어받아 작업합니다.',                   tag: '시작하기',   icon: '⚡', to: '/docs/run-agent' },
  { t: '변경 사항 리뷰',             d: 'agent가 작업을 마치면, 무엇을 배포할지는 사람이 결정합니다.',                          tag: '일상 작업',  icon: '◇', to: '/docs/agents/reviewing' },
...
const WHATS_NEW_KO: ChangelogItem[] = [
  { date: '2026-05-08', title: '이제 여러 agent가 동시에 작업해도 변경 사항이 충돌하지 않습니다.', kind: 'feature' },
  { date: '2026-04-30', title: 'Claude Code와 함께 Codex(OpenAI)도 정식 agent 옵션으로 추가됐습니다.', kind: 'release' },
  { date: '2026-04-22', title: '더 똑똑해진 트리아지 — 할 일이 해당 채널에서 가장 활발한 agent로 자동 배정됩니다.', kind: 'feature' },
...
const WELCOME_HERO_KO = (
  <>
    <div className="rhpd-actions">
      ...
      <LL className="rhp-btn-ghost" to="/docs/agents/overview">agent가 작동하는 방식</LL>
    </div>
...
const PAGES_KO: DocPage[] = [
  // ============================================================ 시작하기
  {
    ...
    lede: (
      <>RunHQ는 팀의 AI 코딩 agent가 실제로 일하는 곳입니다. 회의에서든, Slack 스레드든, 이메일이든, 사이트의 위젯이든 — 어디서나 할 일을 만들면 agent가 이어받아 변경을 작성하고, diff를 다시 사람의 리뷰로 넘깁니다. 워크스페이스 하나. 받은편지함 하나. 모든 기록이 한곳에 남습니다.</>
    ),
        ...
        body: (
          <>
            <Steps>
              ...
              <li>선택 사항: 폴더 이름을 지정합니다. agent가 코드를 체크아웃하는 디스크상의 작업 디렉터리입니다.</li>
        ...
        body: (
          <>
            ...
            <UL>
              ...
              <li><Em>Browser</Em> — agent가 웹을 탐색해야 하는 작업용입니다.</li>
              <li><Em>Chat</Em> — 이 프로젝트의 agent와 자유롭게 대화하는 공간입니다.</li>
            </UL>
...

  {
    ...
    group: 'Get started',
    title: '첫 할 일 만들기',
    lede: <>할 일은 RunHQ의 작업 단위입니다. 팀의 누구나 만들 수 있습니다. 나중에 agent가 이어받거나, 직접 실행해도 됩니다.</>,
    sections: [
      {
        id: 'add',
        heading: '할 일 추가',
        body: (
          <>
            ...
            <P>할 일은 <Pill kind="pending">대기</Pill> 상태로 채널에 등록됩니다. agent가 이어받으면 다음 단계로 넘어갑니다.</P>
          </>
        ),
      },
      {
        ...
        body: (
          <UL>
            ...
            <li><Em>첨부 파일</Em> — 스크린샷은 거의 항상 agent가 더 나은 결과를 내게 해줍니다. 에러 메시지도 마찬가지입니다.</li>
          </UL>
        ),
      },
      {
...

  {
    ...
    group: 'Get started',
    title: '첫 agent 실행하기',
    lede: <>할 일이 있다면, agent를 돌리는 데는 클릭 한 번이면 됩니다. 무엇이 펼쳐지는지 살펴보세요.</>,
    sections: [
      {
        id: 'run',
        heading: 'Run 누르기',
        body: (
          <>
            <Steps>
              ...
              <li>agent를 고르거나 추천된 agent를 그대로 사용합니다 — RunHQ는 기본적으로 해당 채널에서 가장 활발한 agent를 추천합니다.</li>
      ...
      },
      {
        ...
        body: (
          <>
            ...
            <UL>
              <li><Em>Terminal</Em> — agent의 실시간 셸 세션. 작업이 진행되는 동안 출력이 스트리밍됩니다.</li>
              <li><Em>Chat</Em> — 사용자와 agent의 대화. 실행 중에도 끼어들어 추가 컨텍스트를 전달할 수 있습니다.</li>
              <li><Em>Files</Em> — 현재까지의 diff. agent가 편집할 때마다 쌓여 갑니다.</li>
            </UL>
        ...
        body: (
          <>
            ...
            <UL>
              ...
              <li><Em>Request changes</Em> — 코멘트를 남기면 agent가 다시 시도합니다.</li>
              <li><Em>Discard</Em> — 작업을 폐기하고 새 할 일을 만듭니다.</li>
            </UL>
            <P>전체 리뷰 흐름은 <NL to="/docs/agents/reviewing">변경 사항 리뷰</NL>를 참고하세요.</P>
          </>
        ),
      },
    ],
  ...
  // ============================================================ 할 일
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: (
          <>
            <P>이미지, PDF, 로그, zip 파일을 설명에 바로 드래그하세요. agent는 텍스트와 함께 첨부 파일을 봅니다 — 특히 스크린샷이 큰 도움이 됩니다.</P>
...

  {
    ...
    group: 'Todos',
    title: '할 일의 라이프사이클',
    lede: <>agent가 작업하는 동안 할 일은 소수의 상태를 거쳐 움직입니다. 대부분의 전환은 자동으로 일어나며, 직접 표시해야 하는 경우는 몇 가지뿐입니다.</>,
    sections: [
      {
        id: 'states',
        heading: '상태 종류',
        body: (
          <>
            <UL>
              ...
              <li><Pill kind="progress">진행 중</Pill> — agent가 활발히 작업 중입니다.</li>
              <li><Pill kind="review">리뷰</Pill> — agent가 작업을 마치고 사람을 기다리고 있습니다.</li>
      ...
      },
      {
        ...
        body: (
          <>
            ...
            <UL>
              <li>agent가 잡을 시작하면 할 일은 <Pill kind="progress">진행 중</Pill>으로 바뀝니다.</li>
              <li>agent가 diff와 함께 멈추면 <Pill kind="review">리뷰</Pill>로 바뀝니다.</li>
...

  {
    ...
    group: 'Todos',
    title: '코멘트와 업보트',
    lede: <>할 일은 대화입니다. 알게 된 컨텍스트를 더하고, 중요한 항목에 투표하고, 실행 중에도 살짝 찔러 agent를 바른 길로 이끄세요.</>,
    sections: [
      {
        id: 'comments',
        heading: '코멘트',
        body: (
          <>
            ...
            <P>agent가 실행되는 동안 추가한 코멘트는 agent의 채팅에도 그대로 전달됩니다. 취소하고 다시 만들 필요 없이 실행 중에 새 정보를 줄 수 있는 방법입니다.</P>
          </>
        ),
      },
      {
      ...
      },
      {
        ...
        heading: '멘션과 알림',
        body: (
          <P><Kbd>@</Kbd>로 누군가를 멘션하면 알림이 전송됩니다(앱 내, 그리고 사용자가 켜둔 경우 이메일로도). agent를 멘션하면 — <Kbd>@</Kbd>agent-name — 트리아저를 건너뛰고 즉시 그 agent에게 할 일이 배정됩니다.</P>
        ),
      },
    ],
  },
    ...
    sections: [
      {
        id: 'todo',
        heading: 'Todo',
        body: <P>작업 큐입니다. 기본 채널 유형이며, 대부분의 프로젝트가 가장 많은 시간을 보내는 곳입니다. 누구나 할 일을 만들 수 있고, agent가 이어받아 실행합니다.</P>,
      },
      {
        id: 'chat',
        heading: 'Chat',
        body: <P>하나 이상의 agent와 자유롭게 대화하는 공간입니다. 브레인스토밍, 질문, 아직 구체적인 작업 단위로 정리되지 않은 일에 사용하세요. 여기서의 대화는 클릭 한 번으로 할 일로 승격할 수 있습니다.</P>,
      },
      {
        id: 'browser',
        heading: 'Browser',
        body: <P>agent가 웹을 탐색해야 하는 작업용입니다 — 리서치, 스크래핑, E2E 테스트. agent는 사용자가 실시간으로 지켜볼 수 있는 관리형 브라우저 세션을 사용합니다.</P>,
      },
      {
        id: 'files',
        heading: 'Files',
        body: <P>프로젝트의 한 폴더로 바로 들어가 보는 화면입니다. agent는 할 일을 거치지 않고 여기서 파일을 읽고 씁니다. 위키 형식의 노트, 설정, 데이터 파일처럼 할 일 모델에 맞지 않는 지속적인 편집에 유용합니다.</P>,
      },
      {
        id: 'workflow',
        heading: 'Workflow',
    ...
    sections: [
      {
        ...
        body: (
          <>
            ...
            <UL>
              <li><Em>버그</Em>와 <Em>기능</Em>을 분리하고 각각에 다른 agent를 두고 싶을 때.</li>
      ...
      },
      {
        ...
        body: <P>웹 페이지를 들여다보는 작업에 agent를 쓰기 시작할 때 — 경쟁사 리서치, E2E 테스트 트리아지, 콘텐츠 점검. 브라우저 채널은 스크린샷과 트레이스를 대화와 함께 보관합니다.</P>,
      },
      {
        id: 'reorder',
  ...
  // ============================================================ agent
  {
    ...
    group: 'Agents',
    title: 'agent란 무엇인가',
    lede: <>agent는 AI인 팀원입니다. 이름, 그 뒤의 모델, 성격(시스템 프롬프트), 능력 집합(도구)을 갖춥니다.</>,
    sections: [
      {
        id: 'pieces',
        heading: '구성 요소',
        body: (
          <UL>
            <li><Em>이름과 아바타</Em> — 팀원들이 agent를 부르는 방식. <Kbd>@</Kbd>name으로 작업을 배정하거나, 채팅에서 호출하거나, 활동 로그에서 확인합니다.</li>
      ...
      {
        id: 'how-many',
        heading: 'agent를 몇 개 만들지',
        body: (
          <>
            <P>대부분의 팀은 <Em>하나나 둘</Em>로 운영합니다. 트리아저는 agent 간 차이가 분명할 때 가장 잘 작동하며, 비슷한 agent가 십여 개 있을 때는 그렇지 않습니다.</P>
            <P>흔한 조합:</P>
            <UL>
              <li>코드 작업용 agent(Claude Code) 하나, 리서치용(브라우저 도구를 갖춘 범용) 하나.</li>
              <li>영역별로 하나씩: 백엔드 agent, 프런트엔드 agent.</li>
              <li>신중하고 깊이 있는 agent(Sonnet) 하나와 빠른 잔일용(Haiku) 하나.</li>
            </UL>
          </>
        ),
      },
      {
        id: 'persistence',
        heading: 'agent가 지속적인 이유',
        body: <P>agent는 잡 사이에 과거 대화의 기억을 가지지 않습니다 — 매 잡은 새 출발입니다. 하지만 <Em>정체성</Em>은 유지됩니다: 배정 이력, 감사 추적, 시간에 따른 성능. 이 연속성이 "이 버그는 백엔드 agent가 가장 잘 다룬다"고 자신 있게 말할 수 있게 해줍니다.</P>,
      },
    ],
  },

  {
    ...
    lede: <>코딩 agent를 운영하는 두 가지 방법. 대부분의 팀은 Claude Code를 선택합니다. 이미 OpenAI에 표준화된 팀이라면 Codex가 잘 맞습니다.</>,
    sections: [
      {
        id: 'claude',
        heading: 'Claude Code',
      ...
      },
      {
        id: 'switching',
        heading: '나중에 바꾸기',
        body: <P>언제든지 agent의 모델을 바꿀 수 있습니다. 다음 잡부터 즉시 새 모델이 적용됩니다. 과거 잡 이력은 agent와 그대로 연결돼 있어, 모델을 바꿔도 초기화되지 않습니다.</P>,
      },
      {
        id: 'both',
        heading: '둘 다 운영하기',
        body: <P>한 agent는 Claude, 다른 agent는 Codex로 두는 데 아무 문제 없습니다. 트리아저는 채널에서 가장 관련 있는 쪽으로 작업을 보내며, 직접 배정해도 됩니다.</P>,
      },
    ],
  },

  {
    path: '/docs/agents/tools',
    group: 'Agents',
    title: 'agent가 사용할 수 있는 도구',
    lede: <>각 agent에는 능력 목록이 있습니다. agent가 자기 영역에 머물길 원할 때 어떤 도구든 꺼둘 수 있습니다.</>,
    sections: [
      {
        id: 'terminal',
        heading: 'Terminal',
        body: <P>agent는 자체 코드 복사본 안에서 셸 명령을 실행할 수 있습니다. 이것으로 빌드하고, 테스트를 돌리고, 패키지를 설치하고, 언어 고유의 도구를 사용합니다. 거의 모든 agent가 이 도구를 켜둡니다.</P>,
      },
      {
        id: 'files',
        heading: 'Files',
        body: <P>프로젝트 작업 디렉터리의 파일을 읽고 씁니다. 코드를 편집하는 모든 agent에 필요합니다. agent는 프로젝트 샌드박스 안의 파일만 보며 — 다른 프로젝트나 홈 폴더에는 접근할 수 없습니다.</P>,
      },
      {
        id: 'browser',
        heading: 'Browser',
        body: <P>관리형 브라우저 세션입니다. 리서치, 스크린샷 기반 QA, 스크래핑, 그리고 실제 웹 페이지를 봐야 하는 작업(상상에 의존하지 않도록)에 유용합니다. agent가 보는 화면을 실시간으로 볼 수 있는 뷰어가 함께 제공됩니다.</P>,
      },
      {
        id: 'http',
        heading: 'HTTP',
        body: <P>agent가 외부 HTTP 요청을 보내 API를 호출하거나, 문서를 가져오거나, 데이터를 받을 수 있습니다. agent가 절대 인터넷에 닿지 않게 하려면 비활성화하세요 — 규제가 엄격한 환경에 유용합니다.</P>,
      },
      {
        id: 'when-disable',
        heading: '언제 도구를 끌까',
        body: (
          <UL>
            <li><Em>제한된 네트워크</Em> — agent 실행에서 외부 트래픽이 제한돼 있다면 HTTP와 Browser를 끄세요.</li>
            <li><Em>속도</Em> — 리서치 전용 agent는 Files나 Terminal이 필요 없습니다; 끄면 더 저렴하고 빨라집니다.</li>
            <li><Em>안전 장치</Em> — 주니어 엔지니어 스타일의 agent에게 처음부터 HTTP를 주지 않는 식으로 운영할 수 있습니다.</li>
          </UL>
        ),
      },
    ],
...

  {
    ...
    group: 'Agents',
    title: '시스템 프롬프트 작성',
    lede: <>시스템 프롬프트는 agent의 성격, 취향, 규칙입니다. 짧고 구체적으로, 그리고 일반적인 코딩 방법이 아니라 팀의 선호에 관해서만 적으세요.</>,
    sections: [
      {
        id: 'good',
      ...
      },
      {
        ...
        body: (
          <UL>
            <li><Em>비밀 정보를 넣지 마세요.</Em> 프롬프트는 agent에 접근 권한이 있는 모두에게 보입니다. 대신 서비스 계정을 사용하세요.</li>
            <li><Em>모델에게 코딩을 가르치려 하지 마세요.</Em> 모델은 이미 코딩할 줄 압니다; 대신 팀의 선호를 알려주세요.</li>
            <li><Em>전부 다 욱여넣지 마세요.</Em> 2,000단어짜리 프롬프트는 agent를 느리고 헷갈리게 합니다. 가차 없이 다듬으세요.</li>
          </UL>
        ),
      },
      {
...

  {
    ...
    group: 'Agents',
    title: '변경 사항 리뷰',
    lede: <>agent가 작업을 마치면, 무엇을 배포할지는 사람이 결정합니다. RunHQ에서 가장 많은 시간을 보내는 곳이 바로 리뷰 탭입니다.</>,
    sections: [
      {
        id: 'open',
        heading: '리뷰 열기',
        body: (
          <>
            ...
            <P>diff에는 agent가 바꾼 모든 파일이 표시됩니다. 어느 줄에든 마우스를 올리면 코멘트를 달 수 있습니다.</P>
          </>
        ),
      },
      {
        ...
        body: (
          <UL>
            ...
            <li><Em>Request changes</Em> — 코멘트를 남기고 되돌립니다. agent는 같은 잡을 이어받아 피드백을 컨텍스트에 포함한 채 다시 시도합니다.</li>
      ...
      },
      {
        id: 'comments',
        heading: '특정 줄에 코멘트',
        body: <P>diff에서 줄을 클릭하면 라인 코멘트를 달 수 있습니다. 이 코멘트는 다음 반복에서 agent에게 그대로 전달됩니다 — "이걸 이름 바꿔라", "X 테스트를 추가해라", "있는 헬퍼를 써라" 같은 지시에 유용합니다.</P>,
      },
      {
        id: 'retry',
        heading: '다른 agent로 재시도',
        body: <P>때로는 정답이 "다른 agent에게 맡겨 보자"일 때가 있습니다. diff 화면에서 <Em>Retry with…</Em>를 클릭해 다른 agent를 고르세요. 새 잡은 원래 할 일과 모든 리뷰 코멘트를 그대로 물려받습니다.</P>,
      },
    ],
  },
    ...
    sections: [
      {
        ...
        body: (
          <UL>
            ...
            <li><Em>작업 폴더</Em> — agent가 코드를 체크아웃하는 디스크 위치. 대부분의 팀은 기본값을 그대로 사용합니다.</li>
          </UL>
        ),
      },
      {
        ...
        body: (
          <>
            <P>샌드박스는 이 프로젝트 안에서 agent의 터미널이 무엇을 할 수 있는지를 결정합니다. 새 프로젝트는 기본적으로 샌드박스가 켜진 상태로 시작합니다 — agent는 작업 폴더 안에서만 읽고 쓸 수 있으며, 바깥은 건드릴 수 없습니다.</P>
...

  {
    ...
    group: 'Projects',
    title: '작업 폴더와 워크트리',
    lede: <>agent가 실행될 때 코드를 그 자리에서 편집하지 않습니다. 자체 복사본을 가지고 작업합니다. 디스크에서 일어나는 일을 이해할 수 있도록 그 구조를 설명합니다.</>,
    sections: [
      {
        id: 'why',
        ...
        body: (
          <>
            <P>두 가지 이유입니다:</P>
            <UL>
              <li><Em>충돌 방지.</Em> 두 agent가 동시에 작업해도 같은 파일을 두고 다투지 않습니다.</li>
              <li><Em>로컬 상태 보존.</Em> agent는 에디터의 열린 파일이나 진행 중인 git 변경을 절대 건드리지 않습니다.</li>
            </UL>
          </>
        ),
      },
      {
        ...
        body: (
          <>
            <P>모든 잡은 자체 <Em>워크트리</Em>로 시작합니다 — 같은 리포의 별도 체크아웃과 자체 브랜치를 가집니다. agent는 그 안에서 동작합니다.</P>
      ...
      },
      {
        ...
        body: (
          <UL>
            ...
            <li><Em>agent 실행</Em> — 모든 읽기와 쓰기는 이 격리된 복사본을 통해 일어납니다.</li>
      ...
      },
      {
        ...
        body: (
          <UL>
            ...
            <li>트리아저가 해당 작업에 가장 잘 맞는 agent를 고릅니다 — <NL to="/docs/widget/triager">트리아저가 agent를 고르는 방식</NL>을 참고하세요.</li>
            <li>자동 실행을 켜둔 경우 agent가 즉시 시작합니다. 아니라면 할 일이 큐에 대기합니다.</li>
          </UL>
        ),
      },
      {
...
<script>
...

  {
    path: '/docs/widget/triager',
    group: 'The widget',
    title: '트리아저가 agent를 고르는 방식',
    lede: <>위젯을 통해 할 일이 도착하면 RunHQ는 agent를 점수화해 하나를 고릅니다. 트리아저가 무엇을 보는지 — 그리고 어떻게 재정의할지 살펴보세요.</>,
    sections: [
      {
        id: 'scoring',
        heading: '점수 계산 방식',
        body: (
          <>
            <P>채널의 허용 목록에 있는 각 agent에 대해 트리아저는 다음을 살핍니다:</P>
            <UL>
              <li><Em>최근 활동</Em> — 이 agent가 최근 이 채널에서 얼마나 자주 일했는가.</li>
              <li><Em>도구 적합도</Em> — 이 작업에 필요해 보이는 도구를 agent가 갖고 있는가.</li>
              <li><Em>메시지와의 일치도</Em> — 할 일 내용을 agent의 프롬프트와 이력에 비추어 빠르게 평가합니다.</li>
            </UL>
            <P>가장 높은 점수를 받은 agent가 작업을 가져갑니다. 동점이면 가장 최근에 활동한 쪽이 우선합니다.</P>
          </>
        ),
      },
      {
        id: 'allowlist',
        heading: '채널별 허용 목록',
        body: <P>기본적으로 프로젝트의 모든 agent가 후보입니다. 특정 agent만 위젯에서 들어온 피드백을 다루게 하려면 채널 설정에서 허용 목록을 지정하세요 — 그 agent들만 후보가 됩니다.</P>,
      },
      {
        id: 'auto',
        heading: '자동 실행과 리뷰',
        body: (
          <UL>
            <li><Em>리뷰 모드(기본)</Em> — 트리아저가 agent를 골라 할 일을 배정합니다. 사람이 직접 Run을 누릅니다.</li>
            <li><Em>자동 실행 모드</Em> — agent가 즉시 시작합니다. 빠른 회전이 필요한 낮은 위험 채널에 적합합니다.</li>
          </UL>
        ),
      },
      {
        id: 'override',
        heading: '트리아저 재정의',
        body: <P>팀원 누구나 언제든지 할 일을 수동으로 다른 agent에게 재배정할 수 있습니다 — 할 일을 열고 <Em>Reassign</Em>을 클릭하면 됩니다. 트리아저는 수동 배정을 덮어쓰지 않습니다.</P>,
      },
    ],
  },
    ...
    sections: [
      {
        ...
        body: (
          <UL>
            ...
            <li><Em>이벤트 기반 작업</Em> — 실패한 배포가 웹훅을 발사하고, agent가 실패를 트리아지합니다.</li>
    ...
    sections: [
      {
        ...
        body: (
          <UL>
            ...
            <li><Em>Admin</Em> — 워크스페이스 설정, 멤버, agent, 통합. 결제 변경을 제외한 거의 모든 일을 할 수 있습니다.</li>
      ...
      },
      {
        id: 'audit',
        heading: '감사 로그',
        body: <P>의미 있는 모든 변경 — 초대, 역할 변경, agent 편집, 잡 실행 — 은 워크스페이스 감사 로그에 남습니다. <Em>Workspace settings → Audit</Em>에서 찾을 수 있습니다. Owner와 Admin은 읽을 수 있고 Member는 읽을 수 없습니다.</P>,
      },
    ],
  },
];
