...
const PRIVACY_T = {
  ...
  ko: {
    ...
    s1H: '1. 서문',
    s1p1Pre: 'RunHQ Solutions Inc.(이하 "RunHQ," "당사," "우리")는 팀이 제품 피드백을 수집하고, AI 코딩 에이전트를 실행하며, 제안된 코드 변경을 검토하고, 소프트웨어를 배포할 수 있도록 하는 호스팅 서비스인 RunHQ 플랫폼을 운영합니다. 본 개인정보 처리방침은 귀하가 본 플랫폼, 당사의 웹사이트',
    s1p1Em: ' runhq.io',
    ...
    s2p1Mid: '로서 행위합니다. 귀하의 조직이 서비스에 제출하는 워크스페이스 콘텐츠(할 일, 댓글, 코드, 첨부파일, 에이전트 실행 등)에 대하여 당사는 해당 콘텐츠의 컨트롤러인 고객 조직(통상 귀하의 사용자)을 대신하여 ',
    ...
    s3aLi2Body: ' 할 일, 프로젝트, 댓글, 첨부파일, 연동을 통해 연결된 코드베이스, 에이전트 실행 입력 및 출력, 검토 결정, 그리고 귀하가 서비스를 통해 생성하거나 업로드하는 기타 모든 콘텐츠.',
    s3aLi3Label: '결제 정보:',
    ...
    s3bLi1Body: ' IP 주소, 사용자 에이전트, 기기 유형, 운영체제, 브라우저, 언어, IP에서 추정한 대략적 위치.',
    s3bLi2Label: '제품 텔레메트리:',
    s3bLi2Body: ' 이용한 페이지 및 기능, 클릭, 에이전트 실행 메타데이터(소요 시간, 토큰, 상태), 오류 추적, 그리고 이와 유사한 이용 이벤트.',
    ...
    s4H: '4. 정보 이용',
    s4Li1Label: '서비스 운영:',
    s4Li1Body: ' 사용자 인증, 할 일 라우팅, 에이전트 실행, 알림 전송, 워크스페이스 콘텐츠 보존.',
    s4Li2Label: '청구 및 회계:',
    ...
    s6H: '6. AI 및 코딩 에이전트',
    s6p1: '귀하가 에이전트 실행을 트리거하면, 귀하가 선택한 입력값(프롬프트, 관련 컨텍스트, 관련 파일)이 귀하가 선택한 AI 제공업체(예: Anthropic(Claude) 또는 OpenAI(Codex))로 전송됩니다. 해당 제공업체는 당사가 연동 전에 검토한 자체 약관 및 개인정보 보호 약정에 따라 데이터를 처리합니다. 당사는 제공업체가 지원하는 경우 귀하 조직의 학습 제외(no-training) 환경설정을 함께 전달합니다.',
    s6p2: '에이전트 출력값은 귀하의 워크스페이스로 반환되며 귀하의 고객 콘텐츠로 취급됩니다. 당사는 청구, 안정성, 감사 목적을 위해 실행 메타데이터(소요 시간, 상태, 토큰 수)를 기록하며, 입력값과 출력값은 제9항에서 정한 기간 동안 보존합니다.',
